...
class JMotor:
    ...
    def zigzag(self, max_speed=30, cycles=3, step_delay=0.05, step_size=2):
        """
    ...
    Args:
        ...
        cycles (int): How many full left+right oscillations to perform.
        step_delay (float): Delay in seconds between each speed adjustment.
        step_size (int): Increment in duty cycle change per step.
        """
        if max_speed <= 0 or max_speed > 100:
            raise ValueError("max_speed must be between 1 and 100")

    # Start going slightly right first
        left_speed = max_speed
        right_speed = max_speed // 2

        for _ in range(cycles):
        # Gradually shift to left turn
            while right_speed < max_speed:
                right_speed += step_size
                left_speed -= step_size
                self.set_left_motor(True, max(0, min(max_speed, left_speed)))
                self.set_right_motor(True, max(0, min(max_speed, right_speed)))
                time.sleep(step_delay)

        # Gradually shift to right turn
